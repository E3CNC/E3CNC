...
Define the thin bootstrap script (`install.sh`) that handles the initial install steps: sudo check, architecture detection, disk space check, binary download, checksum verification, and handoff to `e3cnc-tui install`.
