...
The test suite SHALL verify installer behavior inside a Debian 12 Docker container. Tests SHALL cover port detection, port conflict, directory migration (single, merge), smart backup, backup pruning, package installation, directory creation, config generation, binary download, 3-screen install wizard, import existing Klipper, update wizard, and rollback.
...
#### Scenario: 3-screen wizard loads
- **WHEN** the install wizard starts in the TUI
- **THEN** the test SHALL verify the loading screen appears and transitions to the decision screen

#### Scenario: Import existing Klipper flow
- **WHEN** a simulated Klipper installation exists in the container
- **THEN** the test SHALL verify the import path detects it and creates the E3CNC management layer

#### Scenario: Update wizard with new version
- **WHEN** a simulated newer version is available
- **THEN** the test SHALL verify the update wizard fetches the changelog and runs the update

#### Scenario: Update auto-rollback
- **WHEN** an update is applied but a critical health check fails
- **THEN** the test SHALL verify the system is rolled back to the previous version