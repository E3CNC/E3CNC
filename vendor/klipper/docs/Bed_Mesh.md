...
It should be noted that each time a BED*MESH_CALIBRATE occurs, the current
state is automatically saved to the \_default* profile. The _default_ profile can be removed as follows:
...
Note that a ZFADE offset does _NOT_ directly apply additional adjustment. It
...
- A "backtracking" check for both the standard probe path and a rapid scan
  ...
  standard probe. Faulty regions _can_ result in backtracking during a rapid
