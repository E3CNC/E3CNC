...
vi.mock('vuetify/components', () => ({
    VDialog: { name: 'VDialog', props: { modelValue: Boolean, width: [String, Number], fullscreen: Boolean }, template: '<div class="v-dialog" v-if="modelValue"><slot /></div>' },
    ...
    VBtn: { name: 'VBtn', props: { variant: String, color: String, icon: [String, Boolean] }, template: '<button class="v-btn" @click="$emit(\'click\')"><slot /></button>' },
...
describe('ConfirmationDialog.vue', () => {
    ...
    it('uses custom cancel button text', () => {
        const wrapper = mount(ConfirmationDialog, {
            props: { modelValue: true, title: 'Confirm', text: 'Are you sure?', actionButtonText: 'Yes', cancelButtonText: 'No' },
            global: { plugins: [i18n] },
        })
