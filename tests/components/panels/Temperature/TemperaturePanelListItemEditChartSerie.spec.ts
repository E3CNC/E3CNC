...
function createStoreWithState(overrides: Record<string, any> = {}) {
    return createStore({
        ...
        getters: {
            'gui/getDatasetValue': () => (params: { name: string; type: string }) => {
                if (params.name === 'extruder' && params.type === 'temperature') return true
                return false
            },
            ...(overrides.getters || {}),
        },
...
describe('TemperaturePanelListItemEditChartSerie.vue', () => {
    beforeEach(() => {
        vi.clearAllMocks()
    })
    ...
    it('renders without crashing', () => {
        ...
        const wrapper = mount(TemperaturePanelListItemEditChartSerie, {
            props: { objectName: 'extruder', serieName: 'temperature' },
            global: { plugins: [store, i18n] },
        })
        expect(wrapper.exists()).toBe(true)
    })

    it('renders a checkbox with the formatted label', () => {
        const store = createStoreWithState()
        const wrapper = mount(TemperaturePanelListItemEditChartSerie, {
            props: { objectName: 'extruder', serieName: 'temperature' },
            global: { plugins: [store, i18n] },
        ...
        expect(checkbox.attributes('data-label')).toContain('Temperature')
    })
    ...
    it('dispatches action on checkbox change', async () => {
        ...
        expect(dispatchSpy).toHaveBeenCalledWith('gui/setChartDatasetStatus', {
            objectName: 'extruder',
            dataset: 'temperature',
            value: false,
        })
