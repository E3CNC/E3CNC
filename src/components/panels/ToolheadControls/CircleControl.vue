...
const stepsXY = computed(() =>
    Array.from(new Set([...(store.state.gui.control?.stepsCircleXY ?? [])])).sort((a, b) => a - b)
)

const stepsZ = computed(() =>
    Array.from(new Set([...(store.state.gui.control?.stepsCircleZ ?? [])])).sort((a, b) => a - b)
)

const isPrinting = computed(() => ['printing'].includes(printer_state.value))

const stepTextClass = computed(() =>
    !homedAxes.value.includes('xy') || isPrinting.value ? ['disabled'] : []
)

const xStepClass = computed(() =>
    !homedAxes.value.includes('x') || isPrinting.value ? ['disabled'] : []
)

const yStepClass = computed(() =>
    !homedAxes.value.includes('y') || isPrinting.value ? ['disabled'] : []
)

const zStepClass = computed(() =>
    !homedAxes.value.includes('z') || isPrinting.value ? ['disabled'] : []
)
